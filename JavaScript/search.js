    ...
    for (var i=0; i<item.length;i++){
        document.getElementById("card").innerHTML +=
            `<div class="col-md-6 mt-3">
                <div class="card p-5 ps-5 pe-5">
                    <h3 class="text-capitalize text-center">${item[i].type}</h3>
                    </br>
                    <div class="figure"><img src="${item[i].src}" width="100%"/></div>
                    <p class="mt-2 text-end"><i>${item[i].price}</i></p>
                    <p class="mt-2">${item[i].desc}</p>
                </div>
            </div>`
    }
};

//create event listener using key up input
document.getElementById("myinput").addEventListener("keyup", function(){
    let text = document.getElementById("myinput").value.toLowerCase();

    filterarray = pastry.filter(function(a){
